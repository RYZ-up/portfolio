...
export default function MetallicPaint({
  ...
  // Backing-store side in device pixels. The canvas is shown letterboxed square
  // inside its box, so only that square matters: at most 1.5x density and capped,
  // because a shader this heavy gains nothing from a 3000x3000 buffer drawn into
  // a 200px box (that was 9M pixels shaded per frame, per canvas).
  const sizeCanvas = useCallback(() => {
    ...
    // Resizing clears the buffer: repaint right away (matters when the loop is
    // paused off-screen or under reduced motion).
    drawRef.current?.();
  }, []);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas || !initGL()) return undefined; // no WebGL: the (decorative) box stays empty

    sizeCanvas();
    setReady(true);

    const resizeObserver = typeof ResizeObserver !== 'undefined' ? new ResizeObserver(sizeCanvas) : null;
    resizeObserver?.observe(canvas);

    // The GPU can drop the context (memory pressure, driver reset): rebuild it
    // instead of leaving a blank panel for good.
    ...
    return () => {
      ...
      textureRef.current = null;
      // Free the GPU context for real when the card goes away (e.g. on a language
      // switch). Skipped while the canvas is still attached: that is React's dev
      // double-mount, which reuses this very canvas.
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    // Depth map is solved in a worker and cached per image (see depthTexture.js).
    loadDepthImageData(imageSrc)
      .then(imgData => {
        if (cancelled) return;
        uploadTexture(imgData);
        setTextureReady(true);
      })
      .catch(error => console.warn('MetallicPaint: could not build the texture', error));
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    if (mouseAnimation) canvas.addEventListener('mousemove', handleMouseMove);

    // The loop only runs while the canvas is on screen, the tab is visible and the
    // visitor hasn't asked for reduced motion; otherwise one still frame is kept.
  ...
  }, [ready, textureReady, mouseAnimation]);
