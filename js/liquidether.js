...
(function () {
  ...
  window.initLiquidEther = function (opts) {
    if (typeof THREE === 'undefined') {
      console.warn('LiquidEther: Three.js is not loaded.');
      return;
    }
...
`;
    ...
    class MouseClass {
      ...
      init(cont) {
        this.container   = cont;
        this.docTarget   = cont.ownerDocument || document;
      ...
      dispose() {
        ...
        this.listenerTarget = this.docTarget = this.container = null;
      }
      isPointInside(cx, cy) {
        if (!this.container) return false;
        const r = this.container.getBoundingClientRect();
        if (r.width === 0 || r.height === 0) return true; // fixed fullscreen — always inside
      ...
      onDocumentMouseMove(e) {
        // For a fixed fullscreen container, always treat as inside
        this.isHoverInside = true;
        if (this.onInteract) this.onInteract();
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          const r  = this.container ? this.container.getBoundingClientRect() : { left:0, top:0, width:window.innerWidth, height:window.innerHeight };
          const w  = r.width  || window.innerWidth;
          const h  = r.height || window.innerHeight;
    ...
    class AutoDriver {
      ...
      update() {
        ...
        if (!this.active) {
          ...
          this.lastTime = this.activationTime = now;
        }
    ...
    class WebGLManager {
      constructor(props) {
        ...
        // Apply simulation options from user config
        ...
        this._io = new IntersectionObserver(entries => {
          ...
          if (this.isVisible && !document.hidden) this.start();
          else this.pause();
        }, { threshold: [0, 0.01, 0.1] });
        this._io.observe(props.$wrapper);
        ...
        this._ro.observe(props.$wrapper);
      }
